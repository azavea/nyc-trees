...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        with connection.cursor() as cursor:
            ...
            # When the database is in RDS, the PostGIS tables are not writable
            # by default.
...
DO $$
    BEGIN
        IF EXISTS (SELECT rolname FROM pg_roles WHERE rolname='rds_superuser') THEN
            ALTER TABLE public.spatial_ref_sys OWNER TO rds_superuser;
        END IF;
    END;
$$;

DELETE FROM public.spatial_ref_sys WHERE srid = 102718;
